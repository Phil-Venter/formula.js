...
exports.DATEVALUE = function (date_text) {
  if (utils.gettype(date_text) !== 'string') {
    return error.value;
  }
...
exports.DAYS = function (end_date, start_date) {
  ...
  if (utils.gettype(end_date) === 'error') {
    return end_date;
  }
  if (utils.gettype(start_date) === 'error') {
    return start_date;
  }
...
exports.HOUR = function (serial_number) {
  ...
  if (utils.gettype(serial_number) === 'error') {
...
exports.INTERVAL = function (second) {
  if (utils.gettype(second) !== 'number' && utils.gettype(second) !== 'string') {
    return error.value;
  } else {
...
exports.NETWORKDAYS.INTL = function (start_date, end_date, weekend, holidays) {
  ...
  if (utils.gettype(weekend) !== 'array') {
    return error.value;
  }
  if (holidays === undefined) {
    holidays = [];
  } else if (utils.gettype(holidays) !== 'array') {
    holidays = [holidays];
  }
  for (let i = 0; i < holidays.length; i++) {
    const h = utils.parseDate(holidays[i]);
    if (utils.gettype(h) === 'error') {
      return h;
    }
    holidays[i] = h;
  }
  const days = (end_date - start_date) / (1000 * 60 * 60 * 24) + 1;
  let total = days;
...
exports.TIMEVALUE = function (time_text) {
  ...
  if (utils.gettype(time_text) === 'error') {
    return time_text;
  }
