...
exports.LEFT = function (text, number) {
  ...
  number = utils.parseNumber(number);
  if (utils.gettype(number) === 'error' || utils.gettype(text) !== 'string') {
    return error.value;
  }
...
exports.MID = function (text, start, number) {
  ...
  number = utils.parseNumber(number);
  if (utils.anyIsError(start, number) || utils.gettype(text) !== 'string') {
    return number;
  }
...
exports.PROPER = function (text) {
  ...
  if (isNaN(text) && utils.gettype(text) === 'number') {
    return error.value;
  }
  if (utils.gettype(text) === 'number') {
    text = '' + text;
  }
...
exports.REPLACE = function (text, position, length, new_text) {
  ...
  if (utils.anyIsError(position, length) ||
    utils.gettype(text) !== 'string' ||
    utils.gettype(new_text) !== 'string') {
    return error.value;
  }
...
exports.SEARCH = function (find_text, within_text, position) {
  let foundAt;
  if (utils.gettype(find_text) !== 'string' || utils.gettype(within_text) !== 'string') {
    return error.value;
  }
  position = (position === undefined) ? 0 : position;
...
exports.T = function (value) {
  return (utils.gettype(value) === 'string') ? value : '';
};
